...
def aa_frmcount(prot_atoms, g2_atoms, dmax, universe, start, end):
    ...
    laa = np.zeros(shape=len(universe.select_atoms("protein and not type H").residues)) 
    # Initialize a numpy array filled with zeros whose dim is no. of protein residues 
    
    br = np.array(universe.select_atoms("protein and not type H").residues) # Store protein residues as given from MD Analysis 
    ...
    for i in fin_res.flat:
        aa_dict[str(list(universe.select_atoms("protein and not type H").residues[i:i+1])[0])] = [laa[i], laa[i]/(end - start)]
...
def grptwocnt_aa(prot_atoms, g2_atoms, dmax, universe):
    ...
    la = np.zeros(shape=len(universe.select_atoms("protein and not type H").residues))
    ...
    for i in fin_res.flat:
        gg_dict[str(list(universe.select_atoms("protein and not type H").residues[i:i+1])[0])] = la[i]
...
def gtwo_trjcnt(prot_atoms, g2_atoms, dmax, universe, start, end):
    ...
    laa = np.zeros(shape=(len(universe.select_atoms("protein and not type H").residues),end-start))

    # Initialize matrix to store amino acid types for each frame
    trj_aa = np.zeros(shape=(len(sf_lb), end-start))
    
    # Initialize array 
    l_final = np.zeros(shape=(len(universe.select_atoms("protein and not type H").residues)))
...
def prot_poly_cntmovie(prot_atoms, g2_atoms, dmax, universe, start, end):
    ...
    assert len(universe.select_atoms("protein and not type H").residues) == len(prot_atoms.residues), "No. of residues in prot_atoms is smaller than total no. of protein residues."
