...
def bavg_pers_cnt(no_of_blks, polymer_atoms, universe, len_bnds, fit_pnts, begin, final):
    """This function takes as inputs:
    - no_of_blks variable that defines the number of blocks
    - polymer_atoms variable, an output from the atom selection class in MD Analysis.
    - the MD Analysis universe variable that contains trajectory information (Make sure you have removed PBC and made molecule whole)
    - len_bnds, an numpy array containing arc length values in Angstroms
    ...
    for i in range(no_of_blks):
        ...
        try:
            ...
            print("Negative cosine theta values present, still doing linear fit...")
            
            # Want to fit a line with no y-intercept
            model_npoly = LinearRegression(fit_intercept=True)
            
            npoly_lc = np.log(cor_tp[0][:fit_pnts])
            
            # fit line to data
            model_npoly.fit(len_bnds[:fit_pnts].reshape(-1,1), npoly_lc)

            # Predict new ln(cos(theta)) values from arc length values
            gg_np = model_npoly.predict(len_bnds[:fit_pnts].reshape(-1,1))
            
            # Residuals between the true y data and model y data 
            resid_np = npoly_lc - gg_np[:fit_pnts]
            
            # How to calculate mean squared error
            mse_p = np.sum(resid_np**2)/(len(resid_np) - 1)
            
            # How to calculate Sum((Xi - avg(X))^2): X values are the arc length values
            nt_ttpp = len_bnds[:fit_pnts] - np.mean(len_bnds[:fit_pnts])
            nhui_pp = nt_ttpp**2
            sum_m = np.sum(nhui_pp)
            
            # How to calculate 95% confidence interval for the slope, N - 1 for dof
            flc_np = scipy.stats.t.ppf(0.95, fit_pnts-1)*np.sqrt(mse_p/sum_m)
            ...
            print("R2 score:", sklearn.metrics.r2_score(npoly_lc[:fit_pnts], gg_np))
            ...
            if fit_pnts == len(len_bnds): 
            
                # Want to fit a line with no y-intercept 
                model_npoly = LinearRegression(fit_intercept=True)
            
                # fit line to data 
                model_npoly.fit(len_bnds.reshape(-1,1), npoly_lc)

                # Predict new ln(cos(theta)) values from arc length values
                gg_np = model_npoly.predict(len_bnds.reshape(-1,1))
                ...
                # How to calculate Sum((Xi - avg(X))^2): X values are the arc length values
                nt_ttpp = len_bnds - np.mean(len_bnds)
                nhui_pp = nt_ttpp**2
                sum_m = np.sum(nhui_pp)
            
                # How to calculate 95% confidence interval for the slope 
                flc_np = stats.t.ppf(0.95, fit_pnts-1)*np.sqrt(mse_p/sum_m)
            ...
            elif fit_pnts != len(len_bnds):
        
                # Want to fit a line with no y-intercept
                model_npoly = LinearRegression(fit_intercept=True)
            
                # fit line to data
                model_npoly.fit(len_bnds[:fit_pnts].reshape(-1,1), npoly_lc[:fit_pnts])
